...
export async function generateOptionReport(
    options: OptionValidationResult[],
    csvPath: string
): Promise<void> {
    const csvString = groupOptionsWithErrors(options);
    await fs.writeFile(csvPath, csvString, "utf-8");
    logger.info(`Report generated: ${csvPath}`);
}

function groupOptionsWithErrors(results: OptionValidationResult[]): string {
