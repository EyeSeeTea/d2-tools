...
import { storePrograms, storeOrganisationUnitsByProgram } from "./quickStoreOperations";
 ...
 */
const getStalePrograms = (apiPrograms, cachedPrograms) => {
...
const getSideEffects = (() => {
    const getOptionSetsOutline = (() => {
        ...
        return programsOutline => programsOutline.flatMap(program => getProgramOptionSets(program));
    })();
...
export const loadPrograms = async () => {
    ...
    const programsDataForSideEffects = (await Promise.all(programBatches.map(loadProgramBatch))).flat(1);
