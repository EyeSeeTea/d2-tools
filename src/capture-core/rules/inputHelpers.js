...
function getDataEntryValidatons(dataEntryMeta) {
...
function getValidDataEntryValues(dataEntryValues, dataEntryValidations) {
...
function convertFormValuesToClientValues(formValues, renderFoundation) {
    return convertFormValuesToClient(formValues, renderFoundation);
}

function getValidFormValues(formValues, fieldsValidation) {
    return Object.keys(formValues)
        .filter(key => fieldsValidation[key])
        .reduce((accValidFormValues, key) => ({ ...accValidFormValues, [key]: formValues[key] }), {});
}

function getFieldsValidationForForm(sectionsFieldsUI, formId) {
...
export function getCurrentClientValues(state, foundation, formId, updatedEventField) {
...
export function getCurrentClientMainData(state, itemId, dataEntryId, foundation) {
