...
export const useMissingCategoriesInProgramSelection = () => {
    ...
    if (selectedProgram && selectedProgram.categoryCombination && !complete) {
        const programCategories = Array.from(selectedProgram.categoryCombination.categories.values()).map(
            ({ id, name }) => ({ id, name })
        );

        return {
            missingCategories: programCategories.filter(
                ({ id }) => !Object.keys(categoriesMeta).some(programCategoryId => programCategoryId === id)
            ),
