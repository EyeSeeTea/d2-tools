...
function addErrorFeedback(state, message, action) {
...
function getErrorFeedback(message, action) {
    return {
        message,
        action,
        feedbackType: "ERROR",
...
export const getFeedbackDesc = appUpdaters =>
    createReducerDescription(
        {
            ...
            [newEventDataEntryActionTypes.SAVE_FAILED_FOR_NEW_EVENT_AFTER_RETURNED_TO_MAIN_PAGE]: (
                ...
                const newState = [...state, getErrorFeedback(i18n.t("Could not save event"))];
                return newState;
            },
