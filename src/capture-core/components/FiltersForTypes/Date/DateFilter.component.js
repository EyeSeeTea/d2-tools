...
const getRelativeRangeErrors = (startValue, endValue, submitAttempted) => {
    ...
    const { error: endValueError } = DateFilter.validateField(
        endValue,
        dataElementTypes.INTEGER_ZERO_OR_POSITIVE
    );
    errors = {
...
class DateFilterPlain extends Component {
    ...
    static isFilterValid(mainValue, fromValue, toValue, startValue, endValue) {
    ...
    static isFromAfterTo(valueFrom, valueTo) {
    ...
    onIsValid() {
        ...
        return (
            !values || DateFilter.isFilterValid(values.main, values.from, values.to, values.start, values.end)
        );
    }
    ...
    handleEnterKeyInTo = value => {
        ...
        if (
            values &&
            !DateFilter.isFilterValid(values.main, values.from, values.to, values.start, values.end)
        ) {
    ...
    setToD2DateTextFieldInstance = instance => {
