...
function getGroupElementsFromScopeId(scopeId) {
    if (!scopeId) {
        return null;
    }
...
export const loadSearchGroupDuplicatesForReviewEpic = (action$, store) =>
    action$.pipe(
        ofType(actionTypes.DUPLICATES_REVIEW, actionTypes.DUPLICATES_REVIEW_CHANGE_PAGE),
        switchMap(({ payload: { page, pageSize, orgUnitId, selectedScopeId, scopeType, dataEntryId } }) => {
            ...
            try {
                ...
                const stream$ = from(getTrackedEntityInstances(queryArgs, attributes));
                return stream$.pipe(
