...
export const StagePlain = ({ stage, events, classes, className, onCreateNew, ...passOnProps }) => {
    const [open, setOpenStatus] = useState(true);
    const { id, name, icon, description, dataElements, hideDueDate, repeatable } = stage;
    return (
        <div data-test="stage-content" className={cx(classes.overview, className)}>
            <Widget
                header={<StageOverview title={name} icon={icon} description={description} events={events} />}
                borderless
