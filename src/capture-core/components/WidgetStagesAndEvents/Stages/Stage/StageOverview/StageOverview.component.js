...
export const StageOverviewPlain = ({ title, icon, description, events, classes }) => {
    ...
    const lastUpdated = Math.max.apply(
        null,
        events.map(e => new Date(e.updatedAt).getTime())
    );
    ...
    return (
        <div className={classes.container}>
            ...
            <div className={classes.indicator}>{i18n.t("{{ totalEvents }} events", { totalEvents })}</div>
            {overdueEvents > 0 ? (
                <div className={cx(classes.indicator, classes.warningIndicator)}>
                    <div className={classes.indicatorIcon}>
                        <IconWarning16 />
                    </div>
