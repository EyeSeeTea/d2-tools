...
const StageDetailPlain = props => {
    ...
    function renderHeader() {
        const headerCells = headerColumns.map(column => (
            <DataTableColumnHeader
                key={column.id}
                name={column.id}
        ...
        return <DataTableRow className={classes.row}>{headerCells}</DataTableRow>;
    }
                    ...
                    >
                        {({ onMouseOver, onMouseOut, ref }) => (
                            <DataTableCell
                                key={id}
                                onClick={() =>
                                    !row.pendingApiResponse && onEventClick(row.id, dataTableProgramStage)
                                }
                                ref={tableCell => {
    ...
    function renderFooter() {
        const renderShowMoreButton = () =>
            events.length > DEFAULT_NUMBER_OF_ROW && displayedRowNumber < events.length ? (
                <Button
                    small
                    secondary
        ...
        const renderResetButton = () =>
            displayedRowNumber > DEFAULT_NUMBER_OF_ROW ? (
                <Button
                    ...
                    className={classes.button}
                    onClick={() => {
                        setDisplayedRowNumber(DEFAULT_NUMBER_OF_ROW);
                    }}
                >
                    {i18n.t("Reset list")}
                </Button>
        ...
        const renderCreateNewButton = () => {
            ...
            return (
                ...
                >
                    <Tooltip content={i18n.t("This stage can only have one event")} closeDelay={50}>
                        {({ onMouseOver, onMouseOut, ref }) => (
                            <div
                                ref={divRef => {
    ...
    if (error) {
        return <div>{i18n.t("Events could not be retrieved. Please try again later.")}</div>;
    }
    return (
        <div className={classes.container}>
            <DataTable className={classes.table}>
                <DataTableHead>{renderHeader()}</DataTableHead>
                <DataTableBody>{renderRows()}</DataTableBody>
                <DataTableFoot>{renderFooter()}</DataTableFoot>
            </DataTable>
        </div>
    );
};

export const StageDetail = withStyles(styles)(StageDetailPlain);
