...
const getAllFieldsWithValue = (eventId, dataElements, dataElementsByType) =>
    dataElements.reduce((acc, { id, type, options }) => {
        const value = dataElementsByType.find(item => item.type === type && item.eventId === eventId)?.ids?.[
            id
        ];
        if (type && value) {
            if (options) {
                if (options[value]) {
                    ...
                    log.error(errorCreator("Missing value in options")({ id, value, options }));
...
const useComputeDataFromEvent = (dataElements, events) => {
...
const useComputeHeaderColumn = (dataElements, hideDueDate) => {
...
const formatRowForView = (row, dataElements) =>
...
export { useComputeDataFromEvent, useComputeHeaderColumn, formatRowForView };
