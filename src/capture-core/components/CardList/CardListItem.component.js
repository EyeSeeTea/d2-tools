...
const deriveEnrollmentType = (enrollments, currentProgramId) => {
    if (!currentProgramId) {
...
const deriveEnrollmentOrgUnitAndDate = (enrollments, enrollmentType, currentProgramId) => {
    if (!currentProgramId) {
        return {};
    }
...
const CardListItemIndex = ({
    ...
    const renderImageDataElement = imageElement => {
    ...
    const { orgUnitName, enrolledAt } = deriveEnrollmentOrgUnitAndDate(
        enrollments,
        enrollmentType,
        currentProgramId
    );
                            ...
                            <Grid item>
                                {item.tei && item.tei.lastUpdated && (
                                    <div className={classes.smallerLetters}>
                                        {i18n.t("Last updated")}{" "}
                                        {item.tei && moment(item.tei.lastUpdated).fromNow()}
                                    </div>
                                )}
...
export const CardListItem = withStyles(getStyles)(CardListItemIndex);
