...
import { withSaveHandler, placements, withCleanUp } from "../../../../DataEntry";
import {
...
const buildRelationshipsSettingsFn = () => {
    ...
    const relationshipsSettings = {
        isApplicable: props => {
            const hasRelationships = props.stage && props.stage.relationshipTypesWhereStageIsFrom.length > 0;
...
class NewEventDataEntry extends Component {
    constructor(props) {
        super(props);
        this.fieldOptions = {
            theme: props.theme,
    ...
    handleSave = (itemId, dataEntryId, formFoundation, saveType) => {
    ...
    renderContent = () => {
        ...
        return (
            <div>
                ...
                <InfoIconText>{this.getSavingText()}</InfoIconText>
            </div>
        );
    };
