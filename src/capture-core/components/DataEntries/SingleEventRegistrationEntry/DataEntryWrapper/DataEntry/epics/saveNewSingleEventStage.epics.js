...
import { removeListItem } from "../../RecentlyAddedEventsList";
...
export const saveNewEventStageEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const saveNewEventInStageLocationChangeEpic = (action$, store, { history }) =>
    action$.pipe(
        ofType(newEventDataEntryActionTypes.REQUEST_SAVE_NEW_EVENT_IN_STAGE),
        map(() => {
...
export const saveNewEventStageFailedEpic = action$ =>
    action$.pipe(
