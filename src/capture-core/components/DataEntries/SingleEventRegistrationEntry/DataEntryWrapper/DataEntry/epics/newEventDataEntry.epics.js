...
export const openNewEventInDataEntryEpic = (action$, store) =>
    action$.pipe(
        ...
        map(() => {
            ...
            return selectionsComplete ? openNewEventInDataEntry() : cancelOpenNewEventInDataEntry();
        })
    );

export const resetRecentlyAddedEventsWhenNewEventInDataEntryEpic = (action$, store) =>
    action$.pipe(
        ofType(
            lockedSelectorActionTypes.NEW_REGISTRATION_PAGE_OPEN,
...
const runRulesForNewSingleEvent = (store, dataEntryId, itemId, uid, orgUnit, history, fieldData) => {
    const state = store.value;
    ...
    return batchActions(
        [updateRulesEffects(effects, formId), rulesExecutedPostUpdateField(dataEntryId, itemId, uid)],
        batchActionTypes.RULES_EFFECTS_ACTIONS_BATCH
    );
};

export const runRulesOnUpdateDataEntryFieldForSingleEventEpic = (action$, store) =>
    action$.pipe(
...
export const runRulesOnUpdateFieldForSingleEventEpic = (action$, store) =>
    action$.pipe(
