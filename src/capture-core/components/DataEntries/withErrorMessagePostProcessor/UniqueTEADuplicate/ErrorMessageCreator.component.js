...
class UniqueTEADuplicateErrorMessageCreatorPlain extends React.Component {
    static renderPrerequisitesError() {
        return <div>{i18n.t("A duplicate exists (but there were some errors, see log for details")}</div>;
    }

    static renderNoAccess(attributeName) {
    ...
    static renderAttributeValueExistsUnsaved(attributeName) {
        const attributeNameLC = attributeName.toLowerCase();
        return (
            <div>
    ...
    renderDefault(attributeName, trackedEntityTypeName, linkButtonClass) {
...
export const UniqueTEADuplicateErrorMessageCreator = withStyles(getStyles)(
    UniqueTEADuplicateErrorMessageCreatorPlain
);
