...
const useCheckForDuplicate = ({ onCheckForDuplicate, searchGroup, scopeType, selectedScopeId }) => {
    if (!searchGroup) {
        return undefined;
    }

    const scopePropertyName =
        scopeType === scopeTypes.TRACKED_ENTITY_TYPE ? "trackedEntityTypeId" : "programId";
    return () =>
        onCheckForDuplicate(searchGroup, {
