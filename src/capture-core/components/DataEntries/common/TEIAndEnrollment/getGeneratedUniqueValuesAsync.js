...
function getFormValue(value, type) {
    return pipe(convertServerToClient, convertClientToForm)(value, type);
}

async function generateUniqueValueAsync(teaId, staticPatternValues) {
    const api = getApi();
...
function getActiveUniqueItemFromCache(id, generatedUniqueValuesCache) {
...
export function getGeneratedUniqueValuesAsync(foundation, generatedUniqueValuesCache, staticPatternValues) {
    if (!foundation) {
...
export const getUniqueValuesForAttributesWithoutValue = async (
    ...
    const uniqueDataElements = foundation
        .getElements()
        .filter(dataElement => dataElement.unique && dataElement.unique.generatable);
    ...
    if (uniqueDataElements && uniqueDataElements.length > 0) {
        ...
        for (const dataElement of uniqueDataElementsWithoutValue) {
            ...
            if (cacheItem) {
                ...
                const generateUniqueValue = await generateUniqueValueAsync(id, staticPatternValues).then(
                    value => ({
                        [id]: value,
                    })
                );
