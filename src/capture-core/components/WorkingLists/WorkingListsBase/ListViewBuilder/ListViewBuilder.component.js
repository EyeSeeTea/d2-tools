...
export const ListViewBuilder = ({ customListViewMenuContents, ...passOnProps }) => {
    const context = useContext(ListViewBuilderContext);
    if (!context) {
        throw Error("missing ListViewBuilderContext");
    }

    const { dataSource, onSelectListRow, onSortList, onSetListColumnOrder, stickyFilters, ...passOnContext } =
        context;
