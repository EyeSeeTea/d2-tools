...
export const initEventListEpic = action$ =>
    action$.pipe(
        ...
        concatMap(action => {
            const { selectedTemplate, columnsMetaForDataFetching, categoryCombinationId, storeId } =
                action.payload;
            const { programId, orgUnitId, categories, lastTransaction, programStageId } =
...
export const updateEventListEpic = action$ =>
    action$.pipe(
...
// TODO: --------------------------------- REFACTOR -----------------------------------
export const requestDeleteEventEpic = action$ =>
    action$.pipe(
