...
function getSelector(key, storeId, isInit) {
    ...
    if (!listSelectors[key]) {
        listSelectors[key] = createSelector(
            ...
            sourceValue =>
                relativeConvertersForPeriods[sourceValue.period] &&
                relativeConvertersForPeriods[sourceValue.period]()
        );
    }
...
function convertCustomRelativeDate(sourceValue) {
...
function convertRelativeDate(sourceValue, key, storeId, isInit) {
    let requestData = [];
...
function convertAbsoluteDate(sourceValue) {
    const requestData = [];
    if (sourceValue.ge) {
...
export function convertDate(sourceValue, key, storeId, isInit) {
...
export function clearMemoization(storeId) {
    selectors[storeId] = {};
}
