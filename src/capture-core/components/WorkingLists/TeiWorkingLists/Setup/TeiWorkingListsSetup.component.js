...
const useCurrentTemplate = (templates, currentTemplateId) =>
    useMemo(
        () =>
            (currentTemplateId && templates.find(template => template.id === currentTemplateId)) ||
            templates[0],
        [templates, currentTemplateId]
    );

const useStaticTemplates = () =>
    useMemo(
        () => [
            {
                id: "default",
...
const useFiltersOnly = ({ enrollment: { enrollmentDateLabel, incidentDateLabel } }) =>
    useMemo(
        () => [
            {
                ...
                transformRecordsFilter: rawFilter => ({
            ...
            },
            {
                ...
                transformRecordsFilter: rawFilter => rawFilter,
            },
        ],
...
const useInjectDataFetchingMetaToLoadList = (defaultColumns, filtersOnly, onLoadView) =>
    useCallback(
        (selectedTemplate, context) => {
            ...
            const filtersOnlyMetaForDataFetching = new Map(
                filtersOnly.map(({ id, type, transformRecordsFilter }) => [
                    id,
