...
const getTeisWithSubvaluesPlain = (querySingleResource, absoluteApiPath) => {
    const getImageOrFileResourceSubvalue = async keys => {
        const promises = keys.map(async key => {
            const { id, displayName: name } = await querySingleResource({
        ...
        return (await Promise.all(promises)).reduce((acc, { id, name }) => {
            acc[id] = name;
            return acc;
        }, {});
    };

    const getOrganisationUnitSubvalue = async keys => {
        const ids = keys.join(",");
        ...
        return organisationUnits.reduce((acc, { id, displayName: name }) => {
            acc[id] = {
                id,
                name,
            };
    ...
    const subvaluePostProcessorByType = {
        [dataElementTypes.IMAGE]: ({ subvalueKey: value, subvalue: name, columnId, teiId }) => ({
            name,
            value,
            url: `${absoluteApiPath}/trackedEntityInstances/${teiId}/${columnId}/image`,
        }),
        [dataElementTypes.FILE_RESOURCE]: ({ subvalueKey: value, subvalue: name }) => ({
            name,
            value,
        }),
    };

    const getSubvalueKeysByType = (clientTeis, columnsWithSubvalues) =>
        columnsWithSubvalues
            ...
            .reduce((acc, { type, subvalueKeys }) => {
                if (subvalueKeys.length > 0) {
                    acc[type] = [...(acc[type] || []), ...subvalueKeys];
                }
                return acc;
            }, {});

    const getSubvaluesByType = subvalueKeysByType => {
        const subvaluePromises = Object.keys(subvalueKeysByType).map(async type => {
    ...
    const addSubvaluesToTeis = (clientTeis, subvaluesByType, columnsWithSubvalues) =>
        columnsWithSubvalues.reduce((teis, { id: columnId, type: columnType }) => {
            const { subvalues = {} } = subvaluesByType.find(({ type }) => type === columnType) || {};
            return teis.map(({ id, record }) => {
    ...
    return async (clientTeis, columnsMetaForDataFetching) => {
        const columnsWithSubvalues = columnsMetaForDataFetching.filter(
            ({ type }) => subvalueGetterByType[type]
        );
...
export const getTeisWithSubvalues = createSelector(
    query => query,
    (query, path) => path,
