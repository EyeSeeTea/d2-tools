...
export const initTeiViewEpic = (action$, store, { querySingleResource, absoluteApiPath }) =>
    action$.pipe(
        ...
        concatMap(action => {
            const { storeId, columnsMetaForDataFetching, filtersOnlyMetaForDataFetching, selectedTemplate } =
                action.payload;
            const { programId, orgUnitId } = action.payload.context;
            return from(
                initTeiWorkingListsViewAsync({
                    programId,
                    orgUnitId,
                    storeId,
                    selectedTemplate,
...
export const updateTeiListEpic = (action$, store, { querySingleResource, absoluteApiPath }) =>
    action$.pipe(
        ofType(workingListsCommonActionTypes.LIST_UPDATE),
        filter(({ payload: { workingListsType } }) => workingListsType === TEI_WORKING_LISTS_TYPE),
        concatMap(action => {
            const { storeId, columnsMetaForDataFetching, filtersOnlyMetaForDataFetching, queryArgs } =
                action.payload;
