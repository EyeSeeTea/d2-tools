...
const convertDate = date => convertServerToClient(date, dataElementTypes.DATE);
...
const getSuggestedDateByNextScheduleDate = (id, eventData) => {
    ...
    if (!possibleNextScheduleValues.length) {
        return undefined;
    }
...
export const useDetermineSuggestedScheduleDate = ({
    ...
    if (!programStageScheduleConfig) {
        return undefined;
    }

    const { nextScheduleDate, standardInterval, generatedByEnrollmentDate, minDaysFromStart } =
        programStageScheduleConfig;
    ...
    const suggestedDate = scheduleDateComputeSteps.reduce(
