...
const compulsoryValidatorWrapper = value => {
...
function buildTypeValidators(metaData) {
...
function buildCompulsoryValidator(metaData) {
...
function buildUniqueValidator(metaData) {
    return metaData.unique
        ? [
              {
                  validator: (value, contextProps) => {
                      if (!value && value !== 0 && value !== false) {
                          return true;
                      }
                      // $FlowFixMe
...
export const getValidators = metaData =>
    [buildCompulsoryValidator, buildTypeValidators, buildUniqueValidator].flatMap(validatorBuilder =>
