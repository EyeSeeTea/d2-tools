...
import { actionTypes as viewEventPageActionTypes } from "../../Pages/ViewEvent/ViewEventComponent/viewEvent.actions";
import { getProgramAndStageFromEvent } from "../../../metaData";

export const loadViewEventDataEntryEpic = (action$, store) =>
    action$.pipe(
        ofType(
        ...
        switchMap(action => {
            ...
            ).pipe(map(item => batchActions(item)));
        })
    );
