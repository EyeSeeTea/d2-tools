...
export const WidgetEventEditPlain = ({ classes, programStage, programStage: { name, icon }, onGoBack }) => {
    const dispatch = useDispatch();
    const currentPageMode = useSelector(
        ({ viewEventPage }) => viewEventPage?.eventDetailsSection?.showEditEvent
    )
        ? pageMode.EDIT
        : pageMode.VIEW;
            ...
            >
                <div className={classes.form}>
                    {currentPageMode === pageMode.VIEW ? (
                        ...
                        <EditEventDataEntry formFoundation={programStage.stageForm} orgUnit={orgUnit} />
                    )}
                </div>
            </Widget>
        </div>
