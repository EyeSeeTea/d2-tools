...
const CommentSectionPlain = ({ placeholder, emptyCommentMessage, comments, handleAddComment, classes }) => {
        ...
        <div className={classes.wrapper}>
            <div className={classes.commentsWrapper}>
                {comments
                    ...
                    .map(comment => (
                        <CommentItem key={`comment-item-${comment.note}-`} {...comment} />
                    ))}
                {comments.length === 0 && <div className={classes.emptyComments}>{emptyCommentMessage}</div>}
            </div>
            ...
            {isEditing && (
                <div className={classes.newCommentButtonContainer} data-test="comment-buttons-container">
                    ...
                    <Button dataTest="cancel-comment-btn" onClick={onCancel}>
                        {i18n.t("Cancel")}
                    </Button>
                </div>
            )}
...
export const CommentSection = withStyles(styles)(CommentSectionPlain);
