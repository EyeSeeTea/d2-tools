...
export const RegistrationDataEntry = ({
    selectedScopeId,
    dataEntryId,
    setScopeId,
    ...
    const dispatchOnSaveWithoutEnrollment = useCallback(
        formFoundation => {
            dispatch(startSavingNewTrackedEntityInstance(formFoundation));
        },
        [dispatch]
    );
    ...
    const dispatchOnSaveWithEnrollment = useCallback(
        formFoundation => {
            dispatch(startSavingNewTrackedEntityInstanceWithEnrollment(formFoundation, teiId));
        },
        [dispatch, teiId]
    );
