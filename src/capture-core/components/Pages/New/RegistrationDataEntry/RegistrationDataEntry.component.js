    ...
    <>
        ...
        {onSave && (
            <div style={{ marginLeft: 8 }}>
                <Button dataTest="create-as-new-person" onClick={onSave} primary>
                    {i18n.t("Save as new")}
                </Button>
            </div>
        )}
...
const RegistrationDataEntryPlain = ({
    ...
    const renderDuplicatesDialogActions = useCallback(
        (onCancel, onSave) => <DialogButtons onCancel={onCancel} onSave={onSave} />,
        []
    );

    const renderDuplicatesCardActions = useCallback(
        ({ item }) => <CardListButton teiId={item.id} orgUnitId={item.tei.orgUnit} />,
        []
    );
        ...
        <>
            {!scopeType && (
                <Paper className={classes.paper}>
                    <div className={classes.title}>{i18n.t("New")}</div>
            ...
            {scopeType === scopeTypes.TRACKER_PROGRAM && (
                <Paper className={classes.paper}>
                    <div className={classes.title}>
                        {teiId
                            ? i18n.t("New Enrollment in program{{escape}} {{programName}}", {
                                  escape: ":",
                                  programName,
            ...
            {scopeType === scopeTypes.EVENT_PROGRAM && <SingleEventRegistrationEntry id="singleEvent" />}
        </>
    );
};
