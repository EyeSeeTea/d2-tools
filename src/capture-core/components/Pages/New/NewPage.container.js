...
const useUserWriteAccess = scopeId => {
    ...
    try {
        if (scope instanceof TrackerProgram) {
            const {
                access,
                trackedEntityType: { access: tetypeAccess },
            } = scope;
            const userHasWriteAccessForTheProgram = access && access.data && access.data.write;
            const userHasWriteAccessForTheTEType =
                tetypeAccess && tetypeAccess.data && tetypeAccess.data.write;
...
export const NewPage = () => {
    ...
    const ready = useSelector(({ activePage }) => !activePage.isDataEntryLoading);

    const currentScopeId = useSelector(
    ...
    const orgUnitSelectionIncomplete = useSelector(
        ({ currentSelections }) => !currentSelections.orgUnitId && !currentSelections.complete
    );

    const newPageStatus = useSelector(({ newPage }) => newPage.newPageStatus);
    ...
    const isUserInteractionInProgress = useSelector(
        state =>
