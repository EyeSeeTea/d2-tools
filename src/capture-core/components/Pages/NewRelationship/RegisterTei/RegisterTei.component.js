...
const RegisterTeiPlain = ({
    ...
    const renderDuplicatesCardActions = useCallback(
        ({ item }) => <CardListButton teiId={item.id} values={item.values} handleOnClick={onLink} />,
        [onLink]
    );

    const renderDuplicatesDialogActions = useCallback(
        (onCancel, onSaveArgument) => (
            <DialogButtons
                onCancel={onCancel}
    ...
    const ExistingUniqueValueDialogActions = useCallback(
        ({ teiId, attributeValues }) => (
            <Button
                dataTest="existing-unique-value-link-tei-button"
                primary
                onClick={() => {
                    onLink(teiId, attributeValues);
                }}
            >
                {i18n.t("Link")}
            </Button>
