...
const WithoutCategorySelectedMessagePlain = ({ programId, classes }) => {
    ...
    const missingCategoriesInSelection = programCategories.filter(
        category => !categories || !categories[category.id]
    );
    const categoryDisplayName = missingCategoriesInSelection[0].name;

    return (
        <div className={classes.incompleteMessageContainer} data-test={"without-category-selected-message"}>
            <IncompleteSelectionsMessage>
                <div className={classes.incompleteMessageContent}>
                    <span>
                        {i18n.t("Please select {{category}}.", {
