...
const convertString = (formValues, dataElement) => {
...
const convertRange = (formValues, dataElement) => {
    const { from, to } = formValues;
    const convertedFrom =
        from && dataElement.convertValue(from, pipeD2(convertFormToClient, convertClientToServer));
    const convertedTo =
        to && dataElement.convertValue(to, pipeD2(convertFormToClient, convertClientToServer));
    if (from || to) {
        return `${dataElement.id}${convertedFrom ? `:ge:${convertedFrom}` : ""}${
            convertedTo ? `:le:${convertedTo}` : ""
        }`;
    }
    return null;
};

const convertOrgUnit = (formValues, dataElement) => {
    const convertedId = dataElement.convertValue(
        formValues,
        pipeD2(convertFormToClient, convertClientToServer)
    );
...
const convertAge = (formValues, dataElement) => {
    const convertedAge =
        formValues &&
...
const convertFile = (formValues, dataElement) => {
    const convertedFileName = dataElement.convertValue(
        formValues,
        pipeD2(convertFormToClient, convertClientToServer)
    );
...
const convertBoolean = (formValues, dataElement) => {
    const convertedBool = dataElement.convertValue(
        formValues,
        pipeD2(convertFormToClient, convertClientToServer)
    );
