...
const isValueContainingCharacter = value => {
    if (!value) {
        return false;
    }
    if (isString(value)) {
    ...
    if (isObject(value)) {
        const numberOfValuesWithLength = Object.values(value)
            .filter(v => isString(v))
            .filter(v => Boolean(v.replace(/\s/g, "").length)).length;
...
const collectCurrentSearchTerms = (searchGroupsForSelectedScope, formsValues) => {
...
const mapStateToProps = (state, { searchGroupsForSelectedScope }) => {
    const {
        formsValues,
...
const mapDispatchToProps = (dispatch, { searchGroupsForSelectedScope }) => ({
