...
export const deriveFilterKeyword = (fieldId, attributes) => {
...
const searchViaAttributesStream = (queryArgs, attributes, triggeredFrom) =>
    from(getTrackedEntityInstances(queryArgs, attributes)).pipe(
        map(({ trackedEntityInstanceContainers: searchResults, pagingData }) => {
            ...
            return showSuccessResultsViewOnSearchPage(searchResults, 1);
        }),
        startWith(showLoadingViewOnSearchPage()),
        catchError(handleErrors)
    );

export const searchViaUniqueIdOnScopeProgramEpic = (action$, store) =>
    action$.pipe(
        ...
        flatMap(({ payload: { formId, programId } }) => {
            ...
            return searchViaUniqueIdStream(queryArgs, attributes, programId);
        })
    );

export const searchViaUniqueIdOnScopeTrackedEntityTypeEpic = (action$, store) =>
    action$.pipe(
        ...
        flatMap(({ payload: { formId, trackedEntityTypeId } }) => {
            ...
            return searchViaUniqueIdStream(queryArgs, attributes);
        })
    );

export const searchViaAttributesOnScopeProgramEpic = (action$, store) =>
    action$.pipe(
...
export const searchViaAttributesOnScopeTrackedEntityTypeEpic = (action$, store) =>
    action$.pipe(
...
export const startFallbackSearchEpic = (action$, store) =>
    action$.pipe(
...
export const fallbackSearchEpic = action$ =>
    action$.pipe(
...
export const fallbackPushPageEpic = (action$, _, { history }) =>
    action$.pipe(
