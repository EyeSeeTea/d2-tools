...
export const ProgramStageSelector = ({ programId, orgUnitId, teiId, enrollmentId }) => {
    const history = useHistory();
    const { tab } = useLocationQuery();
    const { error: enrollmentsError, enrollment } = useCommonEnrollmentDomainData(
        teiId,
        enrollmentId,
        programId
    );
    const { loading: programLoading, error: programError, program } = useProgramFromIndexedDB(programId);

    useEffect(() => {
        if (enrollmentsError || programError) {
            log.error(
                errorCreator("Enrollment page could not be loaded")({
                    enrollmentsError,
                    programError,
                })
            );
    ...
    const programStages = useMemo(
        () =>
            !programLoading &&
            program?.programStages?.reduce((accStage, currentStage) => {
                accStage.push({
                    id: currentStage.id,
                    eventCount: enrollment?.events?.filter(event => event.programStage === currentStage.id)
                        ?.length,
    ...
    return (
        <>
            {program ? (
                <Widget header={i18n.t("Choose a stage for a new event")} noncollapsible>
