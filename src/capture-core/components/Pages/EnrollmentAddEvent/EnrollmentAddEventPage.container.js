...
import { IdTypes, useValidatedIDsFromCache } from "../../../utils/cachedDataHooks/useValidatedIDsFromCache";
...
import {} from "./EnrollmentAddEventPage.types";
...
const EnrollmentAddEventPagePlain = ({ classes }) => {
    const { teiId, programId, orgUnitId, enrollmentId } = useLocationQuery();
    const {
        valid: validIds,
        loading,
        error: validatedIdsError,
    } = useValidatedIDsFromCache({ programId, orgUnitId });
    const {
        enrollment,
        attributeValues,
    ...
    // $FlowFixMe
    const pageIsInvalid =
        (!loading && !Object.values(validIds)?.every(Id => Id?.valid)) ||
        commonDataError ||
        validatedIdsError;
