...
import { convertClientRelationshipToServer, getRelationshipsForEvent } from "../../../../relationships";
import { getRelationshipNewTei } from "../../NewRelationship/RegisterTei";

const relationshipKey = "viewEvent";

export const loadRelationshipsForViewEventEpic = action$ =>
    action$.pipe(
        ofType(
        ...
        switchMap(action => {
            ...
            return getRelationshipsForEvent(event.eventId, event.programId, event.programStageId).then(
                relationships =>
                    batchActions(
...
export const deleteRelationshipForViewEventEpic = (action$, store) =>
    action$.pipe(
...
export const addRelationshipForViewEventEpic = (action$, store) =>
    action$.pipe(
...
export const saveRelationshipAfterSavingTeiForViewEventEpic = action$ =>
    action$.pipe(
...
export const handleViewEventRelationshipSaveTeiFailedEpic = action$ =>
    action$.pipe(
...
export const saveRelationshipFailedForViewEventEpic = action$ =>
    action$.pipe(
        ofType(viewEventRelationshipsActionTypes.SAVE_FAILED_FOR_EVENT_RELATIONSHIP),
        map(action => removeRelationship(relationshipKey, action.meta.clientId))
    );

export const relationshipSavedForViewEventEpic = (action$, store) =>
    action$.pipe(
        ...
        map(action => {
            ...
            const relationshipId =
                action.payload.bundleReport.typeReportMap.RELATIONSHIP.objectReports[0].uid;
