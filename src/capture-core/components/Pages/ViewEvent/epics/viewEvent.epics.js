...
export const getEventOpeningFromEventListEpic = action$ =>
    action$.pipe(
        ...
        switchMap(({ payload: { eventId } }) =>
            getEvent(eventId)
                .then(eventContainer =>
                    eventContainer
                        ? Promise.all([eventContainer, getRulesEngineOrgUnit(eventContainer.event.orgUnitId)])
                        : []
                )
...
export const getEventFromUrlEpic = (action$, store) =>
    action$.pipe(
...
export const getOrgUnitOnUrlUpdateEpic = action$ =>
    action$.pipe(
...
export const openViewPageLocationChangeEpic = (action$, _, { history }) =>
    action$.pipe(
        ofType(eventWorkingListsActionTypes.VIEW_EVENT_PAGE_OPEN),
        map(({ payload: { eventId } }) => {
...
export const backToMainPageEpic = (action$, store) =>
    action$.pipe(
        ...
        map(() => {
            ...
            const listId =
                state.workingListsTemplates.eventList && state.workingListsTemplates.eventList.currentListId;
...
export const backToMainPageLocationChangeEpic = (action$, store, { history }) =>
    action$.pipe(
        ofType(viewEventActionTypes.START_GO_BACK_TO_MAIN_PAGE),
        switchMap(() => {
            const state = store.value;
...
export const openAddRelationshipForViewEventEpic = action$ =>
    action$.pipe(
