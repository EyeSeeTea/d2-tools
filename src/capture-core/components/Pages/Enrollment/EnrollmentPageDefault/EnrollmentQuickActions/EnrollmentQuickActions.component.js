    ...
    const noStageAvailable = useMemo(
        () =>
            stagesWithEventCount.every(
                programStage => !programStage.repeatable && programStage.eventCount > 0
            ),
        [stagesWithEventCount]
    );

    const onNavigationFromQuickActions = tab => {
        history.push(
            `/enrollmentEventNew?${buildUrlQueryString({ programId, teiId, enrollmentId, orgUnitId, tab })}`
        );
    };
