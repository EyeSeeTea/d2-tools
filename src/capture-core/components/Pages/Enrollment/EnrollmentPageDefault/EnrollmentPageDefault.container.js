...
export const EnrollmentPageDefault = () => {
    ...
    if (programMetaDataError || enrollmentsError) {
        log.error(
            errorCreator("Enrollment page could not be loaded")({ programMetaDataError, enrollmentsError })
        );
    }
    ...
    const onViewAll = stageId => {
        history.push(`/enrollment/stageEvents?${buildUrlQueryString({ orgUnitId, programId, stageId })}`);
    };
    ...
    const onEventClick = (eventId, stageId) => {
        history.push(
    ...
    const onUpdateTeiAttributeValues = useCallback(
        (updatedAttributeValues, teiDisplayName) => {
