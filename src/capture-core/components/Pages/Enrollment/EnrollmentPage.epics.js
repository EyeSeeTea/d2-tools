...
const sortByDate = (enrollments = []) =>
    enrollments.sort((a, b) => moment.utc(b.enrolledAt).diff(moment.utc(a.enrolledAt)));
...
const fetchTeiStream = (teiId, querySingleResource) =>
    from(querySingleResource(teiQuery(teiId))).pipe(
...
export const fetchEnrollmentPageInformationFromUrlEpic = action$ =>
    action$.pipe(
...
export const startFetchingTeiFromEnrollmentIdEpic = (action$, store, { querySingleResource }) =>
    action$.pipe(
        ...
        flatMap(() => {
            ...
            return from(querySingleResource({ resource: "tracker/enrollments", id: enrollmentId })).pipe(
                ...
                catchError(() => {
                    const error = i18n.t('Enrollment with id "{{enrollmentId}}" does not exist', {
                        enrollmentId,
                    });
                    return of(showErrorViewOnEnrollmentPage({ error }));
                }),
...
export const startFetchingTeiFromTeiIdEpic = (action$, store, { querySingleResource }) =>
    action$.pipe(
...
export const openEnrollmentPageEpic = (action$, store, { querySingleResource, history }) =>
    action$.pipe(
