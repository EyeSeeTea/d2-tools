...
export const useCommonEnrollmentDomainData = (teiId, enrollmentId, programId) => {
    ...
    const fetchedEnrollmentData = {
        reference: data,
        enrollment: data?.trackedEntityInstance?.enrollments?.find(
    ...
    useEffect(() => {
        if (fetchedEnrollmentData.reference) {
            dispatch(
                setCommonEnrollmentSiteData(
                    fetchedEnrollmentData.enrollment,
                    fetchedEnrollmentData.attributeValues.map(({ attribute, value }) => ({
                        id: attribute,
                        value,
                    }))
                )
