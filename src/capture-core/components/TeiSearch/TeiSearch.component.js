...
class TeiSearchPlain extends React.Component {
    constructor(props) {
        super(props);
        this.state = { programSectionOpen: true };
    }

    getFormId = searchGroupId => {
    ...
    handleSearch = (formId, searchGroupId) => {
        const { id } = this.props;
        this.props.onSearch(formId, searchGroupId, id);
    };

    handleSearchResultsChangePage = pageNumber => {
    ...
    renderSearchForms = searchGroups => (
