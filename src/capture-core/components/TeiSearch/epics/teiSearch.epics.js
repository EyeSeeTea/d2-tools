...
const searchTei = (state, searchId, formId, searchGroupId, pageNumber, resultsPageSize) => {
    const currentTeiSearch = state.teiSearch[searchId];
    const formValues = state.formsValues[formId];

    const { selectedProgramId, selectedTrackedEntityTypeId, selectedOrgUnit, selectedOrgUnitScope } =
        currentTeiSearch;
...
export const teiSearchChangePageEpic = (action$, store) =>
    action$.pipe(
...
export const teiSearchEpic = (action$, store) =>
    action$.pipe(
