...
class SearchFormPlain extends React.Component {
    ...
    validateForm() {
        if (!this.formInstance) {
            log.error(errorCreator(SearchFormPlain.errorMessages.SEARCH_FORM_MISSING)({ Search: this }));
            return {
                error: true,
                isValid: false,
            };
    ...
    render() {
        ...
        if (!searchForm) {
            return <div>{SearchFormPlain.errorMessages.SEARCH_FORM_MISSING}</div>;
        }
        const searchButtonText = searchGroup.unique
            ? this.getUniqueSearchButtonText(searchForm)
            : i18n.t("Search by attributes");
        return (
            <div data-test="d2-form-area" className={classes.container}>
                <D2Form
