...
const useComponentLifecycle = orgUnit => {
    ...
    useEffect(() => {
        pageFetchesOrgUnit &&
            orgUnit &&
            dispatch(setCurrentOrgUnitBasedOnUrl({ id: orgUnit.id, name: orgUnit.name }));
    }, [dispatch, orgUnit, pageFetchesOrgUnit]);
};

export const LockedSelector = ({
    ...
    const lockedSelectorLoads = useSelector(({ activePage }) => activePage.lockedSelectorLoads);
    ...
    const dispatchOnOpenNewRegistrationPageWithoutProgramId = useCallback(() => {
        const actions = [resetProgramIdBase(), openNewRegistrationPageFromLockedSelector()];
    ...
    const dispatchOnResetProgramId = useCallback(
        baseAction => {
            const actions = [baseAction, ...customActionsOnProgramIdReset];
    ...
    const organisationUnits = useSelector(({ organisationUnits: orgUnits }) => orgUnits);
