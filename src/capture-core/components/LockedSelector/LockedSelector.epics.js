...
export const setOrgUnitIdEpic = (action$, store, { history }) =>
    action$.pipe(
        ...
        switchMap(({ payload: { orgUnitId, pageToPush } }) => {
            ...
            if (programId) {
                const programContainsOrgUnitId =
                    programCollection.get(programId)?.organisationUnits[orgUnitId];
...
export const setProgramIdEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const resetProgramIdEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const startAgainEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const getOrgUnitDataBasedOnUrlUpdateEpic = (action$, store, { querySingleResource }) =>
    action$.pipe(
        ...
        switchMap(action => {
            ...
            return from(querySingleResource(orgUnitsQuery(action.payload.nextProps.orgUnitId))).pipe(
                flatMap(response =>
                    of(
                        setCurrentOrgUnitBasedOnUrl({
                            id: response.id,
...
export const setOrgUnitDataEmptyBasedOnUrlUpdateEpic = action$ =>
    action$.pipe(
        ofType(lockedSelectorActionTypes.FROM_URL_UPDATE),
        filter(action => !action.payload.nextProps.orgUnitId),
...
export const validateSelectionsBasedOnUrlUpdateEpic = action$ =>
    action$.pipe(
        ofType(
...
export const fetchOrgUnitEpic = (action$, _, { querySingleResource }) =>
    action$.pipe(
        ...
        switchMap(({ payload: { orgUnitId } }) =>
            from(querySingleResource(orgUnitsQuery(orgUnitId))).pipe(
                map(({ id, displayName: name, code }) => setCurrentOrgUnitBasedOnUrl({ id, name, code }))
            )
        ),
        catchError(() => of(errorRetrievingOrgUnitBasedOnUrl(i18n.t("Could not get organisation unit"))))
    );

export const resetTeiSelectionEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const setEnrollmentSelectionEpic = (action$, store, { history }) =>
    action$.pipe(
...
export const resetEnrollmentSelectionEpic = (action$, _, { history }) =>
    action$.pipe(
