...
const EmptyPrograms = ({ classes, handleResetOrgUnit }) => {
    ...
    return (
        <Paper square elevation={0} className={classes.paper}>
            ...
            <div className={classes.noProgramsContainer}>
                {i18n.t("No programs available.")}
                <LinkButton className={classes.programsHiddenTextResetOrgUnit} onClick={handleResetOrgUnit}>
                    {i18n.t("Show all")}
                </LinkButton>
            </div>
        </Paper>
...
class ProgramSelectorPlain extends Component {
    constructor(props) {
        super(props);
    ...
    areAllProgramsAvailable(programOptions) {
        return (
            programOptions.length === this.programsArray.filter(program => program.access.data.read).length
        );
    }
    ...
    renderWithSelectedProgram(selectedProgram) {
                                            ...
                                            return (
                                                <div className={classes.selectedText}>
                                                    ...
                                                    <IconButton
                                                        ...
                                                        onClick={() =>
                                                            this.handleResetCategoryOption(category.id)
                                                        }
                                                    >
                                                        <IconCross24 />
                                                    </IconButton>
        ...
        return (
            <Paper
                square
                elevation={0}
                className={this.props.classes.selectedPaper}
                data-test="program-selector-container"
            >
                {this.renderSelectedProgram(selectedProgram)}
            </Paper>
        );
    ...
    render() {
        ...
        if (programOptions.length === 0) {
            return (
                <EmptyPrograms classes={this.props.classes} handleResetOrgUnit={this.props.onResetOrgUnit} />
            );
        }
