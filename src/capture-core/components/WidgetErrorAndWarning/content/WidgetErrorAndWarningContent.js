...
const ObjectItem = ({ rule, listItem }) => (
    <li key={rule.id} className={listItem}>
        {rule.message}
    </li>
);

const StringItem = ({ message, listItem }) => <li className={listItem}>{message}</li>;

const WidgetErrorAndWarningContentPlain = ({ widgetData, type, classes }) => {
    const warning = type === widgetTypes.WARNING;
    const error = type === widgetTypes.ERROR;
    return (
        <div data-test="widget-content" className={classes.widgetWrapper}>
            <ul className={classes.unorderedList}>
                {widgetData?.map((message, index) => {
