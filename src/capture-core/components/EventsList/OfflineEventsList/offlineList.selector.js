...
import { getStageForEventProgram, OptionSet, Option, DataElement } from "../../../metaData";
...
const createMainPropertyOptionSet = column => {
...
export const makeColumnsSelector = () =>
    createSelector(programIdSelector, columnsOrderStateSelector, (programId, columnsOrderFromState) => {
        const stageContainer = getStageForEventProgram(programId);
        if (stageContainer.error) {
            return columnsOrderFromState;
        }

        const stageForm = stageContainer.stage.stageForm;
...
const createEventsContainer = (events, eventsValues, sortOrder) =>
...
export const makeCreateEventsContainer = () =>
    createSelector(eventsMainDataSelector, eventsValuesSelector, sortOrderSelector, createEventsContainer);

const onIsEventsEqual = (
    { eventContainers: prevEventContainers },
    { eventContainers: currentEventContainers }
) =>
...
const buildWorkingListData = (eventsContainer, columns) => {
