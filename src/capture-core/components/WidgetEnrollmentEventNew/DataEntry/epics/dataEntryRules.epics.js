...
const runRulesForNewEvent = (
    ...
    return batchActions(
        [updateRulesEffects(ruleEffects, formId), rulesExecutedPostUpdateField(dataEntryId, itemId, uid)],
        newEventWidgetDataEntryBatchActionTypes.RULES_EFFECTS_ACTIONS_BATCH
    );
};

export const runRulesOnUpdateDataEntryFieldForNewEnrollmentEventEpic = (action$, store, { history }) =>
    action$.pipe(
        ofType(newEventWidgetDataEntryBatchActionTypes.UPDATE_DATA_ENTRY_FIELD_ADD_EVENT_ACTION_BATCH),
        map(actionBatch =>
            actionBatch.payload.find(
                action => action.type === newEventWidgetDataEntryActionTypes.RULES_ON_UPDATE_EXECUTE
            )
        ),
        map(action => {
            const { dataEntryId, itemId, uid, orgUnit, rulesExecutionDependenciesClientFormatted } =
                action.payload;
...
export const runRulesOnUpdateFieldForNewEnrollmentEventEpic = (action$, store, { history }) =>
    action$.pipe(
