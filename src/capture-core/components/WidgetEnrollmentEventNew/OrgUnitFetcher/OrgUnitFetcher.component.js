...
export const OrgUnitFetcher = ({ orgUnitId, ...passOnProps }) => {
    const { error, orgUnit } = useOrganisationUnit(orgUnitId, "displayName,code");
    const groups = useOrgUnitGroups(orgUnitId);

    if (error) {
        return <div>{i18n.t("organisation unit could not be retrieved. Please try again later.")}</div>;
    }

    if (orgUnit && groups) {
        orgUnit.groups = groups;

        return <Validated {...passOnProps} orgUnit={orgUnit} />;
    }
