...
const getValidElementConfigsVisiblePrioritized = columns =>
    new Map(
        columns
...
const splitBasedOnHasValueOnInit = (elementConfigs, filtersWithValueOnInit) => {
...
const fillUpIndividualElements = (elementConfigs, occupiedSpots) => {
...
const getUserSelectedElements = (elementConfigs, userSelectedFilters) => {
...
const renderIndividualFilterButtons = ({
    ...
    [...(filtersOnly || []), ...individualElementsArray].map(
        ({ id, type, header, options, multiValueFilter }) => (
            <div
                key={id}
...
const renderRestButton = (restElementsArray, onSelectRestMenuItem) =>
...
const FiltersPlain = memo(props => {
    ...
    const elementsContainer = React.useMemo(() => {
        ...
        const { fillUpElements, remainingElements: remainingElementsAfterFillUp } = fillUpIndividualElements(
    ...
    }, [columns, stickyFilters, filtersOnlyCount]);
    ...
    return <React.Fragment>{filterButtons}</React.Fragment>;
});
