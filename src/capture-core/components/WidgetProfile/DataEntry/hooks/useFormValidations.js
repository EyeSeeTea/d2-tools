...
export const useFormValidations = (dataEntryId, itemId, saveAttempted) => {
    ...
    const {
        ...
        rulesEffectsGeneralWarnings:
            store.rulesEffectsGeneralWarnings && store.rulesEffectsGeneralWarnings[ruleId],
    ...
    const fieldsValidated = useMemo(
        () => Object.values(formsSectionsFieldsUI).every(({ valid }) => valid === true),
        [formsSectionsFieldsUI]
    );
    const rulesValidated = useMemo(
        () =>
            // $FlowFixMe[incompatible-use]
            Object.values(rulesEffectsMessages).every(
                ({ error, errorOnComplete }) => !error && !errorOnComplete
            ) &&
    ...
    const warningsMessages = prepareMessages(
        rulesEffectsGeneralWarnings,
        rulesEffectsMessages,
        saveAttempted,
        "warning"
