...
const subValueGetterByElementType = {
    // todo (report lgmt)
    [dataElementTypes.FILE_RESOURCE]: (value, eventId, metaElementId) => {
    ...
    [dataElementTypes.IMAGE]: (value, eventId, metaElementId) => {
        const baseUrl = config.baseUrl;
        return getApi()
            .get(`fileResources/${value}`)
            .then(res => ({
    ...
    [dataElementTypes.ORGANISATION_UNIT]: (value, eventId, metaElementId) => {
...
export async function getSubValues(eventId, programStage, values) {
    if (!values) {
        return null;
    }
