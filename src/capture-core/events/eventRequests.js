...
function getConvertedValue(valueToConvert, inputKey) {
    let convertedValue;
...
async function convertToClientEvent(event) {
...
export async function getEvent(eventId) {
    const api = getApi();
    const apiRes = await api.get(`tracker/events/${eventId}`);
...
export async function getEvents(queryParams) {
    ...
    const eventContainers =
        apiRes && apiRes.instances
            ? await apiRes.instances.reduce(async (accEventsPromise, apiEvent) => {
