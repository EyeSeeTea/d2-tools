...
function getValuesById(apiDataValues) {
...
function convertDataValues(apiEvent) {
    ...
    const convertedDataValues =
        stageForm.convertValues && stageForm.convertValues(dataValuesById, convertValue);
    return convertedDataValues;
}

function convertMainProperties(apiEvent) {
...
function convertToClientEvent(event) {
...
export const prepareEnrollmentEventsForRulesEngine = (apiEvents = [], currentEvent) =>
    apiEvents.map(apiEvent =>
        currentEvent && currentEvent.eventId === apiEvent.event
            ? currentEvent
