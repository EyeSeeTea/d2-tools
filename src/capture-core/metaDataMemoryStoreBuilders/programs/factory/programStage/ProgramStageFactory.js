...
export class ProgramStageFactory {
    ...
    constructor({ cachedOptionSets, cachedRelationshipTypes, locale }) {
    ...
    async _buildSection(cachedProgramStageDataElements, sectionSpecs) {
        ...
        if (sectionSpecs.dataElements) {
            ...
            await sectionSpecs.dataElements.asyncForEach(async sectionDataElement => {
                ...
                if (!cachedProgramStageDataElement) {
                    log.error(errorCreator("could not find programStageDataElement")({ sectionDataElement }));
                    return;
                }
    ...
    async _buildMainSection(cachedProgramStageDataElements) {
        const section = new Section(o => {
    ...
    static _convertProgramStageDataElementsToObject(cachedProgramStageDataElements) {
        if (!cachedProgramStageDataElements) {
            return {};
        }
    ...
    async build(cachedProgramStage, programId) {
        const stage = new ProgramStage(_stage => {
            ...
            _stage.relationshipTypes = this.relationshipTypesFactory.build(programId, cachedProgramStage.id);
            ...
            _stage.stageForm = new RenderFoundation(_form => {
                ...
                _form.addLabel({
                    id: "occurredAt",
                    label: cachedProgramStage.executionDateLabel || "Report date",
                });
                _form.addLabel({
                    id: "scheduledAt",
                    label: cachedProgramStage.dueDateLabel || "Scheduled date",
                });
        ...
        if (cachedProgramStage.formType === "CUSTOM" && cachedProgramStage.dataEntryForm) {
            ...
            await cachedProgramStage.programStageSections.asyncForEach(async section => {
